    ...
    <div
        ...
        <div
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 text-left">
                <div class="flex-1">
                    <p class="text-[0.75rem] uppercase tracking-[0.25em] text-purple-200 mb-2">New Task</p>
                    <h1 class="text-2xl font-extrabold text-white sm:text-3xl">Add a Task</h1>
                    <p class="text-[1rem] text-slate-300 mt-1 sm:mt-2 sm:text-base">Give it a title and optional details.</p>
                </div>
                <a href="{{ route('index') }}"
        ...
        <div
            class="w-full max-w-3xl bg-white/5 border border-white/10 backdrop-blur rounded-2xl p-6 sm:p-8 shadow-[0_20px_80px_-30px_rgba(88,80,236,0.45)]">
            <form action="{{ route('addTask') }}" method="POST" class="grid gap-4 sm:gap-5 relative">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Add details, notes, or links"
                        class="mt-2 w-full rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-sm text-white shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400/40 sm:py-2.5 sm:text-base">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="flex flex-wrap gap-3 mt-4">
                    ...
                    <a href="{{ route('index') }}"
                        class="w-full sm:w-auto flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-white/10">
                        Cancel
                    </a>
                </div>
